        ...
        <div class="card">
            <div class="card-body">
                <div class="d-lg-flex align-items-center mb-4 gap-3">
                    {{-- <form method="GET"  action="{{ route('admin.color.search') }}">add --}}
                        <div class="d-lg-flex">
                            <div class="col-8">
                ...
                {{-- </div> --}}
                     <div class="ms-auto"><a href="{{route('admin.color.create')}}" class="btn btn-primary radius-30 mt-2 mt-lg-0"><i class="bx bxs-plus-square"></i>Add Colors</a></div>
                </div>
                @if($Colors->count()>0)
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                {{-- <td style="vertical-align: middle">
                                    <button class="btn btn-sm btn-{{ $Color->status ? 'success' : 'danger' }}" data-id="{{ $Color->id }}" id="toggle-status-btn">
                                        {{ $Color->status ? 'Deactivate' : 'Active' }}
                                    </button>
                                </td> --}}
                                ...
                                <td style="vertical-align: middle">
                                    <div class="d-flex order-actions">
                                        <a href="{{route('admin.color.edit', $Color->id)}}" class=""><i class='bx bxs-edit'></i></a>
                                        <form method="post" action="{{ route('admin.color.destroy', $Color->id) }}" class="delete-color-form">
                                            @csrf
                                            {{ method_field('DELETE') }}
                                            <button type="button" class="ms-2 delete-button" data-id="{{ $Color->id }}" style="padding: 9px 10px; outline:none; border:none; border-radius:5px;">
